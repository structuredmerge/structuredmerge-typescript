...
export const SESSION_DIAGNOSTICS_TRANSPORT_VERSION = 1;

export type SessionDiagnosticsTransportImportErrorCategory =
  | 'kind_mismatch'
  | 'unsupported_version';

export interface SessionDiagnosticsTransportImportError {
  readonly category: SessionDiagnosticsTransportImportErrorCategory;
  readonly message: string;
}

export interface SessionDiagnosticsEnvelope {
  readonly kind: 'template_directory_session_diagnostics';
  readonly version: typeof SESSION_DIAGNOSTICS_TRANSPORT_VERSION;
  readonly diagnostics: SessionDiagnosticsReport;
}

export function sessionDiagnosticsEnvelope(
...
export function importSessionDiagnosticsEnvelope(value: unknown): {
  diagnostics?: SessionDiagnosticsReport;
  error?: SessionDiagnosticsTransportImportError;
} {
  if (
    !value ||
    typeof value !== 'object' ||
    (value as { kind?: unknown }).kind !== 'template_directory_session_diagnostics'
  ) {
    return {
      error: {
        category: 'kind_mismatch',
  ...
  if (envelope.version !== SESSION_DIAGNOSTICS_TRANSPORT_VERSION) {
    return {
      error: {
        category: 'unsupported_version',
        message: `unsupported template_directory_session_diagnostics envelope version ${String(envelope.version)}.`
      }
    };
  }

  return { diagnostics: envelope.diagnostics };
}
...
export const SESSION_OUTCOME_TRANSPORT_VERSION = 1;

export type SessionOutcomeTransportImportErrorCategory = 'kind_mismatch' | 'unsupported_version';

export interface SessionOutcomeTransportImportError {
  readonly category: SessionOutcomeTransportImportErrorCategory;
  readonly message: string;
}

export interface SessionOutcomeEnvelope {
  readonly kind: 'template_directory_session_outcome';
  readonly version: typeof SESSION_OUTCOME_TRANSPORT_VERSION;
  readonly outcome: SessionOutcomeReport;
}

export function sessionOutcomeEnvelope(outcome: SessionOutcomeReport): SessionOutcomeEnvelope {
  return {
...
export function importSessionOutcomeEnvelope(value: unknown): {
  outcome?: SessionOutcomeReport;
  error?: SessionOutcomeTransportImportError;
} {
  if (
    !value ||
    typeof value !== 'object' ||
    (value as { kind?: unknown }).kind !== 'template_directory_session_outcome'
  ) {
    return {
      error: {
        category: 'kind_mismatch',
  ...
  if (envelope.version !== SESSION_OUTCOME_TRANSPORT_VERSION) {
    return {
      error: {
        category: 'unsupported_version',
        message: `unsupported template_directory_session_outcome envelope version ${String(envelope.version)}.`
      }
    };
  }
...
export const SESSION_REQUEST_TRANSPORT_VERSION = 1;

export type SessionRequestTransportImportErrorCategory = 'kind_mismatch' | 'unsupported_version';

export interface SessionRequestTransportImportError {
  readonly category: SessionRequestTransportImportErrorCategory;
  readonly message: string;
}

export interface SessionRequestEnvelope {
  readonly kind: 'template_directory_session_request';
  readonly version: typeof SESSION_REQUEST_TRANSPORT_VERSION;
  readonly request: SessionRequestReport;
}

export function sessionRequestEnvelope(request: SessionRequestReport): SessionRequestEnvelope {
  return {
...
export function importSessionRequestEnvelope(value: unknown): {
  request?: SessionRequestReport;
  error?: SessionRequestTransportImportError;
} {
  if (
    !value ||
    typeof value !== 'object' ||
    (value as { kind?: unknown }).kind !== 'template_directory_session_request'
  ) {
    return {
      error: {
        category: 'kind_mismatch',
  ...
  if (envelope.version !== SESSION_REQUEST_TRANSPORT_VERSION) {
    return {
      error: {
        category: 'unsupported_version',
        message: `unsupported template_directory_session_request envelope version ${String(envelope.version)}.`
      }
    };
  }
...
export const SESSION_RUNNER_REQUEST_TRANSPORT_VERSION = 1;

export type SessionRunnerRequestTransportImportErrorCategory =
  | 'kind_mismatch'
  | 'unsupported_version';

export interface SessionRunnerRequestTransportImportError {
  readonly category: SessionRunnerRequestTransportImportErrorCategory;
  readonly message: string;
}

export interface SessionRunnerRequestEnvelope {
  readonly kind: 'template_directory_session_runner_request';
  readonly version: typeof SESSION_RUNNER_REQUEST_TRANSPORT_VERSION;
  readonly request: SessionRunnerRequest;
}

export function sessionRunnerRequestEnvelope(
...
export function importSessionRunnerRequestEnvelope(value: unknown): {
  request?: SessionRunnerRequest;
  error?: SessionRunnerRequestTransportImportError;
} {
  if (
    !value ||
    typeof value !== 'object' ||
    (value as { kind?: unknown }).kind !== 'template_directory_session_runner_request'
  ) {
    return {
      error: {
        category: 'kind_mismatch',
  ...
  if (envelope.version !== SESSION_RUNNER_REQUEST_TRANSPORT_VERSION) {
    return {
      error: {
        category: 'unsupported_version',
        message: `unsupported template_directory_session_runner_request envelope version ${String(envelope.version)}.`
      }
    };
  }
...
export const SESSION_RUNNER_PAYLOAD_TRANSPORT_VERSION = 1;

export type SessionRunnerPayloadTransportImportErrorCategory =
  | 'kind_mismatch'
  | 'unsupported_version';

export interface SessionRunnerPayloadTransportImportError {
  readonly category: SessionRunnerPayloadTransportImportErrorCategory;
  readonly message: string;
}

export interface SessionRunnerPayloadEnvelope {
  readonly kind: 'template_directory_session_runner_payload';
  readonly version: typeof SESSION_RUNNER_PAYLOAD_TRANSPORT_VERSION;
  readonly payload: SessionRunnerPayload;
}

export function sessionRunnerPayloadEnvelope(
...
export function importSessionRunnerPayloadEnvelope(value: unknown): {
  payload?: SessionRunnerPayload;
  error?: SessionRunnerPayloadTransportImportError;
} {
  if (
    !value ||
    typeof value !== 'object' ||
    (value as { kind?: unknown }).kind !== 'template_directory_session_runner_payload'
  ) {
    return {
      error: {
        category: 'kind_mismatch',
  ...
  if (envelope.version !== SESSION_RUNNER_PAYLOAD_TRANSPORT_VERSION) {
    return {
      error: {
        category: 'unsupported_version',
        message: `unsupported template_directory_session_runner_payload envelope version ${String(envelope.version)}.`
      }
    };
  }
...
export const SESSION_INSPECTION_TRANSPORT_VERSION = 1;

export type SessionInspectionTransportImportErrorCategory = 'kind_mismatch' | 'unsupported_version';

export interface SessionInspectionTransportImportError {
  readonly category: SessionInspectionTransportImportErrorCategory;
  readonly message: string;
}

export interface SessionInspectionEnvelope {
  readonly kind: 'template_directory_session_inspection';
  readonly version: typeof SESSION_INSPECTION_TRANSPORT_VERSION;
  readonly inspection: SessionInspectionReport;
}

export function sessionInspectionEnvelope(
...
export function importSessionInspectionEnvelope(value: unknown): {
  inspection?: SessionInspectionReport;
  error?: SessionInspectionTransportImportError;
} {
  if (
    !value ||
    typeof value !== 'object' ||
    (value as { kind?: unknown }).kind !== 'template_directory_session_inspection'
  ) {
    return {
      error: {
        category: 'kind_mismatch',
  ...
  if (envelope.version !== SESSION_INSPECTION_TRANSPORT_VERSION) {
    return {
      error: {
        category: 'unsupported_version',
        message: `unsupported template_directory_session_inspection envelope version ${String(envelope.version)}.`
      }
    };
  }

  return { inspection: envelope.inspection };
}
