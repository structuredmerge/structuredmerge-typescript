...
describe('template directory session report fixture', () => {
  ...
  it('conforms to the session-outcome envelope-application fixture', () => {
    const fixturePath = path.resolve(
      process.cwd(),
      '..',
      'fixtures',
  ...
  it('conforms to the session-request transport-rejection fixture', () => {
    const fixturePath = path.resolve(
      process.cwd(),
      '..',
      'fixtures',
    ...
    for (const testCase of fixture.cases) {
      ...
      expect(importSessionRequestEnvelope(envelope)).toEqual({
        error: testCase.expected_error
      });
    }
  });

  it('conforms to the session-request envelope-application fixture', () => {
    const fixturePath = path.resolve(
      process.cwd(),
      '..',
      'fixtures',
  ...
  it('conforms to the session-runner-payload transport-rejection fixture', () => {
    const fixturePath = path.resolve(
      process.cwd(),
      '..',
      'fixtures',
    ...
    for (const testCase of fixture.cases) {
      const envelope = resolveSessionRunnerPayloadEnvelopeFixturePaths(
        testCase.envelope,
        fixtureRoot
      );
      expect(importSessionRunnerPayloadEnvelope(envelope)).toEqual({
        error: testCase.expected_error
      });
    }
  });

  it('conforms to the session-runner-payload envelope-application fixture', () => {
    const fixturePath = path.resolve(
      process.cwd(),
      '..',
      'fixtures',
  ...
  it('conforms to the session-runner-request transport-envelope fixture', () => {
    const fixturePath = path.resolve(
      process.cwd(),
      '..',
      'fixtures',
    ...
    for (const testCase of fixture.cases) {
      const input = resolveRunnerRequestFixturePaths(testCase.input, fixtureRoot);
      const expected = resolveSessionRunnerRequestEnvelopeFixturePaths(
        testCase.expected_envelope,
        fixtureRoot
      );

      expect(sessionRunnerRequestEnvelope(input as any)).toEqual(expected);
      expect(importSessionRunnerRequestEnvelope(expected)).toEqual({ request: input });
    }
  });
  ...
  it('conforms to the session-command-payload envelope-application fixture', () => {
    const fixturePath = path.resolve(
      process.cwd(),
      '..',
      'fixtures',
    ...
    for (const testCase of fixture.cases) {
      ...
      const imported = importSessionCommandPayloadEnvelope(envelope);
      expect(imported.error).toBeUndefined();
      expect(
        runTemplateDirectorySessionCommandPayload(
...
function resolveSessionRunnerRequestEnvelopeFixturePaths(
  envelope: Record<string, unknown>,
  fixtureRoot: string
) {
  const cloned = JSON.parse(JSON.stringify(envelope)) as Record<string, unknown>;
